...
void DImageGrid::OnRender(Drawing* drawing)
{
    ...
    for (int y = 0; y < (int)rect.bottom;)
    {
        ...
        {
            fRect rcDraw(x, y, x + RECSIZE, y + RECSIZE);
...
void ImageEditPanel::DrawPanelBk(Drawing* drawing)
{
    ...
    for (int y = 0; y < (int)rect.bottom;)
    {
        for (int x = iBeg; x < (int)rect.right;)
        {
            fRect rect(x, y, x + RECSIZE, y + RECSIZE);
...
{
    _editBmp = bmp;
    _cornerRect = bmp->GetViewCorner();
    _viewBox = bmp->GetViewbox();

    // 图像实际绘制区域
    _rcViewbox = bmp->GetViewbox();
    // 图像四个边角
    _rcViewCorner = bmp->GetViewCorner();
    // 图像绘制区域像素单位
...
suic::Rect ImageEditPanel::UpdateViewbox(suic::fRect rect, int iMode)
{
    _viewBox = rect;
    ...
    InvalidateVisual();
    return _viewBox.ToRect();
}

suic::Rect ImageEditPanel::UpdateViewcorner(suic::Rect rect, int iMode)
{
    _cornerRect = rect;
...
void ImageEditPanel::Restore()
{
    ...
    _editBmp->SetViewcorner(_rcViewCorner);
...
void ImageEditPanel::Commit()
{
    ...
    _editBmp->SetViewcorner(_cornerRect);
}
