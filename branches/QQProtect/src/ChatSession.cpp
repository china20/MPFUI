    ...
    if (_chatSession != NULL && _chatSession->GetDisplayMode() == eDisplayMode::eDisplayBubble)
    {
        ...
        if (iFirstChatInfo >= 0)
        {
            ...
            for (; i < _chatSession->GetCount(); ++i)
            {
                ...
                //begPoint = SystemParameters::TransformFromDevice(begPoint.x, begPoint.y).ToPoint();
                //endPoint = SystemParameters::TransformFromDevice(endPoint.x, endPoint.y).ToPoint();
