	...
	REOBJECT *preobj)	//@parm	the data to use for initialization
{
	...
	_extent = preobj->sizel;	// COMPATIBILITY ISSUE: the RE1.0 code had some
    ...
    //
    // TBL:这里一定要大于等于0才认为有效，否则
    // 对于Ojbect宽和高很大时，一个SHORT长度不够
    //
	if (preobj->sizel.cx > 0 && preobj->sizel.cy > 0)
	{
		_extent = preobj->sizel;
	}
	else
	{
		FetchObjectExtents();
...
{
    if (!pdp->GetWordWrap() || !pdp->TxObjectWrapping())
    {
        _vRatio = 1.0f;
    }
    else
    {
        // tbl
        LONG vWid = pdp->HimetricXtoDX(_extent.cx);
        //xLeft = pdp->GetViewWidth() - xLeft - 1;
        xLeft = max(0, xLeft);

        if (xLeft < vWid)
        {
            _vRatio = (float)xLeft / (float)vWid;
        }
        else
        {
            _vRatio = 1.0f;
    ...
    return GetObjSize(pdp, xWidth, yHeight, yDescent);
}

void COleObject::GetObjSize(const CDisplay *pdp, LONG& xWidth, LONG& yHeight, SHORT yDescent)
{
    xWidth = pdp->HimetricXtoDX(_extent.cx);
    yHeight = pdp->HimetricYtoDY(_extent.cy);

    CorereViewSizel(xWidth, yHeight);
	...
	RECT  *prcRender)       //@parm pointer to render rectangle
{
    ...
    if (!(rc.left > prcRender->right || rc.right < prcRender->left))
    {
...
void COleObject::FetchObjectExtents(void)
{
	...
	{	
		...
		if( !(_pi.dwFlags & REO_GETMETAFILE) )
		{
			...
			if( hr == NOERROR )
			{
				hr = poo->GetExtent(_pi.dvaspect, &_extent);
				poo->Release();
			}
		...
		if( hr != NOERROR )
		{
			...
			{
				hr = pvo->GetExtent(_pi.dvaspect, -1, NULL, &_extent);
				pvo->Release();
			}
		}
		...
		if( hr != NOERROR || _extent.cx == 0 || _extent.cy == 0 )
		{
			_extent.cx = _extent.cy = 2000;
		}
	}
    ...
    LONG yHeight = pdp->HimetricYtoDY(_extent.cy);
    LONG xWidth = pdp->HimetricXtoDX(_extent.cx);

    CorereViewSizel(xWidth, yHeight);

	_rcPos.right = _rcPos.left + xWidth;
...
{
	CDisplay *	pdp = _ped->_pdp;
	SIZEL		sizelold = _extent;

	// Change the size of our internal representation.
	_extent.cx = pdp->DXtoHimetricX(rcNew.right - rcNew.left);
	_extent.cy = pdp->DXtoHimetricX(rcNew.bottom - rcNew.top);

	//If the size didn't really change, don't do anything else.
	if (_extent.cx != sizelold.cx || _extent.cy != sizelold.cy)
	{
...
void COleObject::CorereViewSizel(LONG& w, LONG& h) const
{
    w = _vRatio * w;
    h = _vRatio * h;
}
	 ...
	 LONG *pAdjust)		//@parm output adjustment needed for positioning below baseline
{
	CRchTxtPtr rtp(_ped, 0);
	POINT pt, pt1;
    LONG yHeight = _ped->_pdp->HimetricYtoDY(_extent.cy);
    LONG xWidth = _ped->_pdp->HimetricXtoDX(_extent.cx);
												...
												//	is currently being activated
{
	...
	_extent = sizelsave;
	poo->SetExtent(_pi.dvaspect, &_extent);
...
DontRunAndSetAgain:
	...
	if(hr == NOERROR && _fIsPaintBrush)
	{
		...
		_fPBUseLocalSizel = !(sizelChk.cx == _extent.cx && 
								sizelChk.cy == _extent.cy);
