...
// V-GUYB (15540): Minimize rounding errors.
#define TwipsFromHimetric(_hm)	(((((long) (_hm)) * (long) 72) + (long) 63) / (long) 127)
#define HimetricFromTwips(_tw)	(((((long) (_tw)) * (long) 127) + (long) 36) / (long) 72)
...
typedef struct _rtfobject
{
	...
	LONG	xExtGoal, yExtGoal;	// @field desired dimensions in twips for pictures
...
struct FontDecorations
{
    LPSTR pszName;
    BYTE  charset;
};

// Array set up in rtfread.cpp
extern const FontDecorations fontDec[];
