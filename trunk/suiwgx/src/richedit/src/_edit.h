...
class CTxtEdit : public ITextServicesEx, public IRichEditOle, public ITextDocument
{
public:
	...
	CTxtEdit(ITextHostEx *phost, IUnknown *punkOuter);
	 ~CTxtEdit ();
...
//	LONG		TxGetSelectionBarWidth() const;
	void		TxGetViewInset(LPRECT prc, const CDisplay *pdp) const;
    ...
    // tbl begin
    BOOL        TxObjectWrapping() const
    {
        BOOL bWrap = FALSE;
        _phost->TxObjectWrapping(bWrap);
        return bWrap;
    }

    HRESULT TxInsertOleObject(LPDATAOBJECT pdo, STGMEDIUM* pStdMedium, FORMATETC* fetc, REOBJECT* reobj)
    {
        return _phost->TxInsertOleObject(pdo, pStdMedium, fetc, reobj);
    }

    HRESULT TxFindOleObject(DWORD cp, REOBJECT* reobj);
    HRESULT TxFindFirstVisibleOleObject();

    HRESULT TxGetPointFromCp(DWORD cp, BOOL leftTop, POINT* point);
    HRESULT TxSetLineLeftOffset(DWORD index, int xLeft);
    HRESULT TxGetLineDetail(DWORD index, int& xCount, int& iInPara, int& xWhite, int& xLeft, int& xWidth, int& xHeight, int* iYPos);
    ...
    virtual HRESULT     TxUpdateCaret();
    // tbl
    virtual HRESULT		TxRecalcView(BOOL bForce);
