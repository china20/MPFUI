	...
	LPTSTR pszSrc)
{
    ...
    cchName=(DWORD)wcslen(pszName)+1;

    if (NULL!=pszSrc)
        cchSrc=(DWORD)wcslen(pszSrc)+1;
    else
    {
        cchSrc=cchName;
        pszSrc=pszName;
    ...
    if (pszName)
    {
        ...
        wcscpy_s((LPTSTR)((LPBYTE)pOD+pOD->dwFullUserTypeName)
            , cchName, pszName);
    }
    else
    ...
    if (pszSrc)
    {
        ...
        wcscpy_s((LPTSTR)((LPBYTE)pOD+pOD->dwSrcOfCopy), cchSrc, pszSrc);
    }
    else
...
)
{
   ...
   BOOL            fLink=FALSE;
    ...
    if (fLink && NULL!=pszName)
	{
		...
		wcscpy_s(szName, TEXT("Linked "));
		wcscpy_s(&(szName[7]), wcslen(pszName), pszName);
	}
    else if (pszName)
       wcscpy_s(szName, pszName);
	else
		szName[0] = 0;
...
)
{
    ...
    //sizelHim.cy = pdp->HimetricYtoDY(sizelHim.cy);
    //sizelHim.cx = pdp->HimetricXtoDX(sizelHim.cx);
	...
	int * dByteUsed)
{
	...
	cchAppendStr = (int)strlen( (char *)szAppendStr );
