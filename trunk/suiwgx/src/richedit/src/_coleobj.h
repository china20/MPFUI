...
public:
	...
	HRESULT InitFromREOBJECT(DWORD cp, REOBJECT *preobj);
										...
										// outgoing calls)
	void 	MeasureObj(const CDisplay *pdp, LONG xMaxWid, LONG& xWidth, LONG& yHeight, SHORT yDescent);
    void 	GetObjSize(const CDisplay *pdp, LONG& xWidth, LONG& yHeight, SHORT yDescent);
	...
	void	ResetSizel(const SIZEL &sizel)
						{_extent = sizel;}
    void    CorereViewSizel(LONG& w, LONG& h) const;
    SIZEL   GetSizel() const
                        { return _extent;};
...
private:
    ...
    float           _vRatio;    //@cmember ratio of width and height for WordWrap
	SIZEL			_extent;	//@cmember the cached "real" size of the object
