	...
	IUndoBuilder &undobldr)		// @parm required to modify the text.
{
    ...
    {
        return CIme::SetCompositionFormPoint(ed);
    }
			...
			if ( psel )
			{
				...
				if ( pCF )
				{
                    ...
                    if ( bFontOK )
					{
                         ...
                         {
                             //lfw = pccs->_lf;
                             pImmSetCompositionFont( hIMC, &(pccs->_lf) );	
                         }
...
BOOL __GetCaretPos(LPPOINT point)
{
    HWND hwnd;
    GUITHREADINFO pg;
	...
	CTxtEdit &ed )	   	// @parm the containing text edit.
{
	...
	{
		SetCompositionFormPoint(ed);
	}
#endif
}

HRESULT CIme::SetCompositionFormPoint(CTxtEdit &ed)
{
    HIMC 				hIMC;
    ...
    {												
        ...
        cf.ptCurrentPos.x = psel->GetXCaret();
        cf.ptCurrentPos.y = psel->GetYCaret();
