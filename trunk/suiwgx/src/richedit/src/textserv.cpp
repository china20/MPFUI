...
//            case EM_STREAMOUT:
    ...
    case WM_IME_COMPOSITION:        // State of user's input has changed
        ...
        if (_fReadOnly && (WORD)wparam != 3)				// Don't allow input if read only,
...
HRESULT CTxtEdit::TxRecalcView(BOOL bForce)
{
    if (bForce)
    {
...
HRESULT CTxtEdit::TxUpdateCaret()
{
    _pdp->UpdateCaretEx();
    return S_OK;
}
...
HRESULT CTxtEdit::TxFindOleObject(DWORD cp, REOBJECT* reobj)
{
    HRESULT hr = S_OK;
    COleObject* oleObj = _pobjmgr->GetObjectFromCp(cp);
    if (oleObj == NULL)
    {
        hr = S_FALSE;
    }
    ...
    {
        hr = oleObj->GetObjectData(reobj, REO_GETOBJ_POLEOBJ/*REO_GETOBJ_ALL_INTERFACES*/);
    }

    return hr;
}

HRESULT CTxtEdit::TxFindFirstVisibleOleObject()
{
    HRESULT hr = S_OK;
    int iLineNo = _pdp->GetFirstVisibleLine();
    LONG cp = 0;
    hr = TxLineIndex((LONG)iLineNo, &cp);
    int index = _pobjmgr->FindIndexForCp(cp);

    return (HRESULT)index;
}

HRESULT CTxtEdit::TxGetPointFromCp(DWORD cp, BOOL leftTop, POINT* point)
{
    HRESULT hr = S_OK;
    CTxtRange txtRange(this, cp, 0);
    if (leftTop)
    {
        txtRange.GetPoint(tomStart | TA_LEFT | TA_TOP, &point->x, &point->y);
    }
    else
    {
        txtRange.GetPoint(tomEnd | TA_BOTTOM | TA_RIGHT, &point->x, &point->y);
    }
    return hr;
}

HRESULT CTxtEdit::TxSetLineLeftOffset(DWORD index, int xLeft)
{
    HRESULT hr = S_OK;
    CLine *	pli = _pdp->TxGetLine(index);
    pli->_xLeft = xLeft;
    return hr;
}

HRESULT CTxtEdit::TxGetLineDetail(DWORD index, int& xCount, int& iInPara, int& xWhite, int& xLeft, int& xWidth, int& xHeight, int* iYPos)
{
    HRESULT hr = S_OK;
    CLine *	pli = _pdp->TxGetLine(index);
    xLeft = pli->_xLeft;
    xWidth = pli->_xWidth;
    xHeight = pli->_yHeight;
    xCount = pli->_cch;
    xWhite = pli->_cchWhite;
    iInPara = pli->_cchEOP;

    if (iYPos != NULL)
    {
        *iYPos = _pdp->YposFromLine(index);
    }
...
STDAPI CreateTextServices(
    ...
    ITextHostEx *phost,       //@parm Client's ITextHost implementation; must be
                            //      valid
