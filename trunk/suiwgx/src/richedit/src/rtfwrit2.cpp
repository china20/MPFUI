...
EC CRTFWrite::WriteRtfObject(RTFOBJECT & rtfObject, BOOL fPicture)
{
	...
	{
		if (*pDim )
			PutCtrlWord(CWF_VAL, *pKeyWordIndex, (LONG)*pDim);
	}
...
EC CRTFWrite::WriteObject(LONG cp, COleObject *pobj)
{
	...
	if (hdata)
	{
		...
		rtfObject.xExt = (LONG) TwipsFromHimetric( reObject.sizel.cx );
		rtfObject.yExt = (LONG) TwipsFromHimetric( reObject.sizel.cy );
...
BOOL CRTFWrite::GetRtfObjectMetafilePict(HGLOBAL hmfp, RTFOBJECT &rtfobject, SIZEL &sizelGoal)
{
	...
	rtfobject.xExtPict = (LONG) pmfp->xExt;
	rtfobject.yExtPict = (LONG) pmfp->yExt;
...
BOOL CRTFWrite::GetRtfObject(REOBJECT &reobject, RTFOBJECT &rtfobject)
{
	...
	if (!fStatic)
	{
		...
		rtfobject.xExt = (LONG) TwipsFromHimetric(sizelObj.cx);
		rtfobject.yExt = (LONG) TwipsFromHimetric(sizelObj.cy);
