...
// Tbl:Object换行时允许的最小宽度
const LONG lObjMinWrapWidth = 8;
		...
		{											// Process next char
			...
			if(ch == WCH_EMBEDDING)
			{
				...
				if( pobj )
				{
                    ...
                    // tbl
                    if (!ped->TxGetWordWrap() || (uiFlags & MEASURE_IGNOREOFFSET) || (0x7fffffff == xWidthMax))
                    {
                        pobj->GetObjSize(_pdp, xAdd, objheight, _yDescent);
                    }
                    else
                    {
                        int iObjWid = xWidthMaxOverhang - _xWidth;
                        if (iObjWid <= 2)
                        {
                            iObjWid = xWidthMax;
                        }

                        pobj->MeasureObj(_pdp, iObjWid, xAdd, objheight, _yDescent);
                    }
