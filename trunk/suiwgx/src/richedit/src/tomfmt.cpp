	...
	ITextPara *pPara)		//@parm New paragraph formatting
{
	...
	if(SameVtables(this, pPara))			// If pPara belongs to this TOM
	...
	DWORD	dwEffect)	//@parm Effect bit to set
{
	...
	if(Value == tomToggle)
	{
		...
		{										//  _CF.dwEffects for toggling
		    ...
		    if (ped)
		    {
		        const CCharFormat *ccf = ped->GetCharFormat(iFormat);
		        if (ccf)
		        {
			        _CF.dwEffects = ccf->dwEffects;
			    }    
		    }   
