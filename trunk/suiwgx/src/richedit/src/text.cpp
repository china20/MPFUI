	...
	TCHAR const *pch)		//@parm text to insert
{
	...
	if(!Count())
	{
        ...
        // 增加CTxtBlk，并使用cbSize初始化第一个CTxtBlk。
    ...
    // 取得新增加的第一个CTxtBlk指针
	ptb = GetRun(0);
    // _cbBlock为CTxtBlk文本块中已分配的字节数
    // ptb->_cch为当前CTxtBlk实际包含的字符数
    // cchInBlock为此CTxtBlk剩余的字符数容量
	cchInBlock = CchOfCb(ptb->_cbBlock) - ptb->_cch;

    // 每个CTxtBlk不能大于49152字节
