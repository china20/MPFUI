	...
	BOOL fFractionalScroll)
{
    ...
    if (yScroll > _yHeight - _yHeightView)
    {
        yScroll = _yHeight - _yHeightView;
    }

    if (xScroll > _xWidth - _xWidthView)
    {
        xScroll = _xWidth - _xWidthView;
    }

    LONG yScrollPos = yScroll;
...
BOOL CDisplayML::DoDeferredUpdateScrollBar()
{
    ...
    BOOL bHoriReturn = UpdateScrollBar(SB_HORZ, TRUE);
    BOOL bVertReturn = UpdateScrollBar(SB_VERT, TRUE);

    return (bVertReturn || bHoriReturn);
    
    // Tbl:这里可能水平更新了，垂直滚动条得不到更新
    // 因此需要改为先更新再返回，这里返回值仅仅为fitcontent用
