...
public:
    ...
    {
        return _ped->TxObjectWrapping();
    }

    virtual CLine*          TxGetLine(int index)
    {
        return NULL;
    }
    virtual LONG    YposFromLine(LONG ili)
    {
        return 0;
    }
	...
	// View recalc and updating
			...
			BOOL	RecalcView (const RECT &rcView, BOOL& bVertBar);
			BOOL	UpdateView();
	...
	virtual void	Zombie();
    virtual INLINE LONG GetMaxYScroll() const { return 0; }
};
