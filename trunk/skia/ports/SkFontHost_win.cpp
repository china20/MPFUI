...
const void* HDCOffscreen::draw(const SkGlyph& glyph, bool isBW,
    ...
    uint16_t glyphID = glyph.getGlyphID() - fBaseGlyphID;
...
private:
 ...
 {
     SkTypeface* skface = SkTypeface::CreateFromName("·ÂËÎ", s);
     return skface;
 }

 SkScalerContext* SkFontHost::CreateScalerContext(SkTypeface* f, const SkDescriptor* desc) {
    return new SkScalerContext_GDI(f, desc);
}
...
unsigned SkScalerContext_GDI::generateGlyphCount() {
    if (fGlyphCount < 0) {
        fGlyphCount = calculateGlyphCount(
                          fDDC, static_cast<const LogFontTypeface*>(this->getTypeface())->fLogFont) + 100;
    }
    return fGlyphCount;
}
...
uint16_t SkScalerContext_GDI::generateCharToGlyph(SkUnichar utf32) {
    ...
    if (SkUTF16_FromUnichar(utf32, (uint16_t*)utf16) == 1) {
         ...
         */
        DWORD result = GetGlyphIndicesW(fDDC, utf16, 1, &index, 0);//GGI_MARK_NONEXISTING_GLYPHS);
        if (result == GDI_ERROR
            || 0xFFFF == index
            || (0x1F == index &&
               (fType == SkScalerContext_GDI::kBitmap_Type ||
...
void SkScalerContext_GDI::generateMetrics(SkGlyph* glyph) {
    ...
    uint32_t ret = GetGlyphOutlineW(fDDC, glyph->getGlyphID(fBaseGlyphCount), GGO_GRAY8_BITMAP | GGO_GLYPH_INDEX, &gm, 0, NULL, &fMat22);
    ...
    UINT glyphId = glyph->getGlyphID(this->fBaseGlyphCount);
...
void SkScalerContext_GDI::generateImage(const SkGlyph& glyph) {
    ...
    fOffscreen.fBaseGlyphID = this->fBaseGlyphCount;
    size_t srcRB;
