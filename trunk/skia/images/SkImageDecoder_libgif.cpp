...
protected:
    virtual int onDecode(SkStream* stream, SkBitmap* bm, Mode mode) SK_OVERRIDE;
...
int SkGIFImageDecoder::onDecode(SkStream* sk_stream, SkBitmap* bm, Mode mode) {
#if GIFLIB_MAJOR < 5
...
void* SkiaGifStaticInit()
{
    return (void*)SkImageDecoder_DecodeReg::Head();
}
