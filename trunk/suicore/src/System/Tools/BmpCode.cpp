...
////////////////////////////////////////////////////////////////////////////////
bool BmpCode::Encode(CxFile * hFile)
{
...
RGBQUAD* GetPalette(suic::Byte* pDib, bool biClrUsed) const
{
    if ((pDib) && (biClrUsed))
    {
        return (RGBQUAD*)(pDib + sizeof(BITMAPINFOHEADER));
    }
    else
    {
        return NULL;
...
bool BmpCode::Decode(FILE * hFile)
{
	if (hFile == NULL) 
    {
        return false;
    ...
    // ∂¡»°bmpÕ∑
    if (fread(&bf, sizeof(bf), 1, hFile) != sizeof(bf))
    {
        //("Not a BMP");
        return false;
    }
	...
	Uint32 dwCompression = bmpHeader.biCompression;
	Uint32 dwBitCount = bmpHeader.biBitCount;
    ...
    if (pRgb)
    {
        if (bIsOldBmp)
        {
            ...
            for (Int32 i = DibNumColors(&head)-1; i >= 0; i--)
            {
        ...
        else 
        {
			...
			for (Uint32 i=0; i < head.biClrUsed; i++) 
            {
