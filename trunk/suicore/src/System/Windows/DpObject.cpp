...
void DpObject::CoerceValue(DpProperty* dp)
{
    ...
    PropMetadata* metadata = NULL;//dp->GetMetadata(RTTIType());
...
//Object* DpObject::InternalSetValue(DpProperty* dp, Object* val, bool coerceWithDefer, bool coerceWithCurr, OperationType operType, bool isInternal)
//{
//    if (NULL == dp)
//    {
//        throw ArgumentNullException(_U("dp"), _U("DpProperty is not null!"), __FILELINE__);
//    }
//
//    PropMetadata* metadata = NULL;
...
//
//        // 是否是一个表达式
//        expr = DynamicCast<Expression>(val);
//
//        if (!expr)
//        {
//            validateFlag = isInternal ? dp->IsValidValueInternal(val) : dp->IsValidValue(val);
//
//            if (!validateFlag)
//            {
...
//                {
//                    throw ArgumentException(_U("InvalidPropertyValue"), __FILELINE__);
//                }
//            }
//        }
//
...
//
//        Expression* oriExpr = entry.IsExpression() ? DynamicCast<Expression>(entry.GetLocalValue()) : NULL;
...
//        if (exprHoldflag)
//        {
//            newEntry.ResetDefaultEffectiveValue(dp, oriExpr);
//            newEntry.SetExprValue(val, oriExpr);
...
//        else
//        {
//            if (coerceWithCurr && (oriExpr != NULL))
//            {
//                oriExpr = NULL;
//            }
//
...
//
//                Object* defValue = metadata->GetDefaultValue();
//
//                // 估算表达式
//                EvaluateExpression(entryIndex, dp, expr, metadata, newEntry);
//
//                if (newEntry.IsExpression() && (newEntry.GetReviseValue()->GetExprValue() == Expression::NoValue))
//                {
//                    //MemberVisitor::SetExprValue(&entry, defValue, expr);
//                    MemberVisitor::SetExprValue(&newEntry, defValue, expr);
//                }
//
...
//        }
//
//        UpdateEffectiveValue(entryIndex, dp, entry, newEntry, coerceWithDefer, coerceWithCurr, operType);
//    }
//
//    return val;
//}
//
//int DpObject::UpdateEffectiveValue(EntryIndex entryi, DpProperty* dp, EffectiveValue oldEntry, EffectiveValue& newEntry
//                                   , bool coerceWithDefer, bool coerceWithCurr, OperationType operType)
//{
//    if (dp == NULL)
//    {
//        throw ArgumentNullException(_U("dp"), __FILELINE__);
...
//
//    // 如果旧值无效，先获取
//    if (oldEntry.GetBaseValueType() == BaseValueType::Unknown)
//    {
//        GetEffectiveValue(entryi, dp, metadata, RequestFlags::RawEntry, oldEntry);
//    }
//    else
//    {
...
//
//    // 优先级不够
//    if (operType != OperationType::ForceUpdate && (newEntry.GetBaseValueType() != BaseValueType::Unknown) && 
...
//
//    Expression* expr = NULL;
//    ObjectPtr controlValue = NULL;
//
//    Object* newValue = NULL;
//    Object* oldValue = oldEntry.GetFlattenedValue(RequestFlags::FullyResolved);
//
//    if (NULL == metadata)
//    {
...
//    if (coerceWithCurr)
//    {
//        controlValue = newEntry.GetValue();
//        newEntry.ResetEffectiveValue(dp, DpProperty::UnsetValue(), oldEntry.GetBaseValueType());
//
//        // 如果新值为表达式，则先进行计算
//        Expression* coerceExr = RTTICast<Expression>(controlValue.get());
//        if (NULL != coerceExr)
//        {
//            //coerceExr->OnAttach(this, dp);
//            coerceExr->GetValue(this, dp, controlValue);
//        }
//
//        if (controlValue.get() != Expression::NoValue)
//        {
//            newEntry.SetCoercedValue(controlValue.get(), true);
//        }
//        else
//        {
//            newEntry.SetCoercedValue(oldValue, true);
//        }
//
...
//    else
//    {
//        bool newAndOldIsExpr = oldEntry.IsExpression() && newEntry.IsExpression();
//        bool newAndOldBaseValueEqual = false;
//
//        // 新值和旧值都是表达式而且基础值相等
//        if (newAndOldIsExpr)
//        {
//            newAndOldBaseValueEqual = newEntry.GetReviseValue()->GetBaseValue() == oldEntry.GetReviseValue()->GetBaseValue();
//        }
//
//        // 估算属性值
//        if (newEntry.GetValue() == DpProperty::UnsetValue() || (newAndOldIsExpr && newAndOldBaseValueEqual))
//        {
//            //
//            // 这里需要禁用表达式的双向绑定赋值
//            // 因为这里的值本身是从表达式估算获得
...
//            {
//                expr = RTTICast<Expression>(oldEntry.GetReviseValue()->GetBaseValue());
//                if (NULL != expr)
//                {
//                    expr->ref();
//                    expr->_enabled = false;
...
//            }
//
//            EvaluateEffectiveValue(entryi, dp, metadata, oldEntry, newEntry, operType);
//        }
//        else
//        {
//            // 如果新值本身是一个表达式，先进行表达式计算
//            expr = RTTICast<Expression>(newEntry.GetValue());
//
//            if (NULL != expr)
//            {
//                expr->ref();
...
//    {
//        SetEffectiveValue(entryi, dp, metadata, newEntry, oldEntry);
//    }
//    else
//    {
//        RemoveEffectiveValue(entryi, dp, metadata);
//    }
//
//    if (NULL == newValue)
//    {
//        newValue = newEntry.GetFlattenedValue(RequestFlags::FullyResolved);
//    }
//
...
//    {
//        isAValueChange = !oldValue->Equals(newValue);
//    }
//
//    if (isAValueChange)
//    {
...
//    {
//        // 值已经改变，触发事件
//        DpPropChangedEventArg e(dp, metadata, oldEntry, newEntry);
