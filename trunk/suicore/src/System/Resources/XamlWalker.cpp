...
/*bool XamlWalker::OnReadingTrack(Track* pTrack, IXamlNode* node)
{
...
bool XamlWalker::OnReadingGridChild(Grid* pGrid, IXamlNode* node)
{
    bool bFind = true;
    String name = node->GetName();
    ...
    {
        ReadGridRowDefs(pGrid, node);
    }
    else if (name.Equals(_U("Grid.ColumnDefinitions")))
    {
        ReadGridColumnDefs(pGrid, node);
    }
    else
    {
        bFind = false;
...
void XamlWalker::ReadGridRowDefs(Grid* pGrid, IXamlNode* pNode)
{
    // 读取子节点
    pNode->Reset();
...
void XamlWalker::ReadGridColumnDefs(Grid* pGrid, IXamlNode* pNode)
{
    // 读取子节点
    pNode->Reset();
